...
SPECS = [
 ('column-reveal','基线柱形展开','bar-chart-grow','bars','各阶段的素材数量','初筛|复核|整理|交付','18,32,46,60','份','柱形从同一零基线依次长出，数字跟随柱顶，最后一项以浅色强调。'),
 ('series-trace','时间序列描线','line-chart-draw','line','分阶段阅读记录','第一周|第二周|第三周|第四周|第五周','12,20,17,30,36','篇','折线逐段绘出，节点与数值跟随到达顺序出现，保留零基线。'),
 ('paired-values','双值前后对照','before-after-stat','compare','两次整理的耗时','首次整理|复用结构','48,18','分钟','两组等宽数值区域先后进入，通过字号与明度区分层级；不自动宣称提升比例。'),
 ('measure-bars','同尺横条比较','horizontal-bar-compare','hbars','两套样例的覆盖量','方案青|方案朱','28,44','项','两根横条共享尺度，由左向右伸展，末端标签同时落定。'),
 ('ordered-podium','排序阶梯榜','top-rank-list','rank','样例条目的阅读次数','案例甲|案例乙|案例丙','320,240,180','次','按输入数值排序，三条排名依次进入，首位通过文字明度强调。'),
 ('inflection-note','拐点旁注','turning-point-line','turn','一个变化节点','阶段一|阶段二|阶段三|阶段四|阶段五','12,16,19,38,45','项','趋势线逐段连接，指定节点亮起细环，解释在图表下方独立一行显示。'),
 ('evidence-footnote','证据脚注条','source-citation-card','citation','预览与适配是两件事','资料：本库接入说明|口径：按渲染框架区分','','','居中的半透明来源卡进入，来源、结论与口径分层显示。'),
 ('duo-overlay','双指标盘点卡','stat-duel','duel','素材盘点','视频|图片','24,36','份','下方居中的半透明信息卡，合计与两项数值依次进入，比例条按真实数据展开。'),
 ...
 ('term-definition','术语边注','concept-spotlight','concept','什么是关键帧','关键帧|记录某一时刻的位置、大小等状态。','','','居中的紧凑注释卡轻抬进入，术语与解释依次显现。'),
 ('step-stair','步骤递进节点','three-step-flow','steps','把素材做成可复用效果','明确用途|替换内容|检查成片','','','三个独立节点水平居中依次进入，连接箭头先于下一个步骤出现，最后同时保持。'),
...
def main():
 ...
 for slug,title,reference,kind,headline,labels,values,unit,description in SPECS:
  ...
  defaults={'title':headline,'labels':labels,'values':values,'unit':unit,
            ...
            'accent':'#dce8e0','background':'#111214','transparent':True,'panelOpacity':82,'focus':2,
...
'''
  ...
  candidates.append({'id':'atelier-'+slug,'title':title,'en':slug,'category':'信息提示','origin':'atelier',
   ...
   'provenance':{'version':'atelier-knowledge-v2','license':'MIT','reference':REFERENCE,'referenceCommit':REF_SHA,'referenceTemplate':reference,'implementation':'independently-authored'}})
